...
from interfacesystech_log.msg import LogRecord
import rospy
import time
import json
...
if __name__ == "__main__":
    rospy.init_node("interfacesystech_logger_client")
    # reset database
    c = MongoClient()
    db = c[rospy.get_param("~database_name", "interfacesystech_log")]
    db["debug"].drop()
    # pub record to logger
    pub = rospy.Publisher("/interfacesystech_log", LogRecord, queue_size=10)
    time.sleep(1)
    record = LogRecord()
